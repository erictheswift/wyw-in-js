 ...
 *
 * Reduced from fibery-ui's calendar/style.ts, whose `borderBottom` /
 * `borderLeft` consts precede a `cx()` and an object-syntax `typeSelectorStyle`;
 * one interpolation there fails under `strategy: "static"` with no
 * `staticResolve` rejection event.
 */
